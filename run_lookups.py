...
# lib_path = r"/user/gent/493/vsc49347/ondemand/data/sys/myjobs/projects/URENIMOD/lib"
lib_path = r"C:/Users/rsegawa/URENIMOD/proposal_1/scripts/lib"
...
from MorphoSONIC.models import UnmyelinatedFiber # C:\Users\...\scripts\lib ihas been already added to system variables
...
def _calculate_instantaneous_membrane_resistance(
        Rm0_ohm_cm,  # Initial membrane resistance (Ω·cm²)
    ...
    Rm_inst = Rm0_ohm_cm * length_diam_factor * thickness_factor
    ...
    for D0_val, d0_val, L0_val, f_val, A_val, Q_val in param_combinations:
        ...
        if A_val == 0:  # No US, use neuron's resting capacitance (already in uF/cm^2)
            ...
            # --- Dynamic Cm calculation for UnmyelinatedFiber ---
                ...
                Rm0 = (rho_m*1e4 * current_initial_membrane_thickness_seg_m*1e2) / (np.pi * (current_initial_length_one_segment_m *1e2) * (current_initial_outer_diameter_seg_m * 1e2))
...
def main():
    ...
    parser.add_argument('-fiber_length', type=float, required=True, help='Fiber length (m)')
    parser.add_argument('-fiber_diameter', type=float, required=True, help='Fiber diameter (m)')
    parser.add_argument('-membrane_thickness', type=float, required=True, help='Membrane thickness (m)')
    parser.add_argument('-freq', type=float, required=True, help='Frequency (Hz)')
    parser.add_argument('-amp', type=float, required=True, help='Amplitude (Pa)')
    parser.add_argument('-charge', type=float, required=True, help='Charge density (C/m²)')
    parser.add_argument('--debug-plot', action='store_true', help='Generate debug plots showing mechanical deformation')
